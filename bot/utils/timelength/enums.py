...
class NumeralType(Enum):
    ...
    - `HUNDRED` — Represents a hundred numerals (ex: a hundred).
    - `THOUSAND` — Represents a thousand numerals (ex: a thousand).
...
class FailureFlags(IntFlag):
    """
    Represents the inputs that will cause parsing to fail.
    ...
    - `MALFORMED_THOUSAND` — A thousand segment was attempted but did not have a leading number or
                            — a proper number of digits following a thousand separator.
    - `MALFORMED_FRACTION` — A fraction was attempted but had more than 2 values, a missing value,
                            — or was not formatted correctly.
    - `MALFORMED_HHMMSS` — An HH:MM:SS was attempted but had more segments than enabled scales or
    ...
    - `CONSECUTIVE_SEGMENTERS` — More than the allowed number of segmenters were parsed in a row.
