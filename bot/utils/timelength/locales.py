...
from bot.utils.timelength.enums import FailureFlags, NumeralType
...
from bot.utils.timelength.parsers.date_parser import DateParserConfig
...
class Locale:
    ...
    - `segmenters` — Return the segmenters if valid.
    ...
    dateconfig: DateParserConfig

    # Date Parsing configs

    def __init__(
        self,
        config_location: str | os.PathLike[str] = "english.json",
        flags: FailureFlags | None = None,  # NOQA
        settings: ParserSettings | None = None,
        dateconfig: DateParserConfig = None,
    ) -> None:
        ...
        self._segmenters: list[str] = self._get_config_or_raise("segmenters", list)
        self._validate_connectors_segmenters()
        ...
        self._date_operators: list[str] = self._get_config_or_raise("date_operators", list)
        self._thousand_delimiters: list[str] = self._get_config_or_raise("thousand_delimiters", list)
        self._date_delimiters: list[str] = self._get_config_or_raise("date_delimiters", list)
    ...
    @property
    def segmenters(self) -> list[str]:
        """Return the segmenters if valid."""
        self._validate_connectors_segmenters()
        return self._segmenters

    @segmenters.setter
    def segmenters(self, value: list[str]) -> None:
        self._segmenters = value
        self._validate_connectors_segmenters()
    ...
    @property
    def date_delimiters(self) -> list[str]:
        """Return the date delimiters if valid."""
        self._validate_delimiters()
        return self._date_delimiters

    @date_delimiters.setter
    def date_delimiters(self, value: list[str]) -> None:
        self._date_delimiters = value
        self._validate_delimiters()

    @property
    def date_operators(self) -> list[str]:
        """Return the date delimiters if valid."""
        self._validate_delimiters()
        return self._date_operators

    @date_operators.setter
    def date_operators(self, value: list[str]) -> None:
        self._date_operators = value
        self._validate_delimiters()
    ...
    def delimiters(self) -> list[str]:
        ...
        self._validate_delimiters()
        return self._hhmmss_delimiters + self._decimal_delimiters + self._thousand_delimiters + self._date_delimiters

    @property
    def specials(self) -> list[str]:
        """Return a list of all special characters."""
        return self._connectors + self._segmenters + self._allowed_terms + self.delimiters
    ...
    def _load_parser(self, base_dir: str) -> Callable[[str, Locale], ParsedTimeLength]:
        ...
        try:
            ...
            if parser := getattr(module, module_name, None):
                return parser
            else:
            ...
            raise InvalidLocaleError(
                self, f"Parser function {module_name!r} not found in parser file: {self._parser_file!r}"
            ) from e
    ...
    def _load_scales(self, scales_json: dict) -> None:
        ...
        def process_scale(_scale_json: dict, scale_name: str) -> None:
            if not scale_name:
                raise InvalidScaleError(scale_name, "A scale key is empty.")
            elif any(key not in _scale_json for key in ["scale", "singular", "plural", "terms"]):
                raise InvalidScaleError(scale_name)
            else:
                try:
                    if isinstance(_scale_json["scale"], str):
                        _scale_json["scale"] = float(Fraction(_scale_json["scale"]))

                    _scale_json["terms"] = tuple(_scale_json["terms"])
                    scale_obj: Scale = Scale(key=scale_name, **_scale_json)
    ...
    def _load_numerals(self, numerals_json: dict) -> None:
        ...
        for numeral_name in numerals_json:
            ...
            elif any(key not in numerals_json[numeral_name] for key in ["type", "value", "terms"]):
            ...
            try:
                ...
                raise InvalidNumeralError(
                    numeral_name, f"An invalid NumeralType was provided for {numeral_name!r}."
                ) from e
            ...
            terms = numerals_json[numeral_name]["terms"]
    ...
    def _validate_connectors_segmenters(self) -> bool:
        """Validate the values for the connectors and segmenters."""

        if set(self._connectors).intersection(self._segmenters):
            raise InvalidLocaleError(self, "Connectors and segmenters may not have overlap.")
    ...
    def _validate_delimiters(self) -> bool:
        ...
        all_delimiters = (
            self._decimal_delimiters + self._thousand_delimiters + self._hhmmss_delimiters + self._date_delimiters
        )
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"{self.__str__()}(config_location={json.dumps(self._config_location)}, "
            f"flags={self.flags!r}, settings={self.settings!r})"
        )
...
class English(Locale):
    ...
    def __init__(
        self, flags: FailureFlags = None, settings: ParserSettings | None = None, dateconfig: DateParserConfig = None
    ) -> None:
        super().__init__("english.json", flags, settings, dateconfig)
...
class Spanish(Locale):
    ...
    def __init__(
        ...
        super().__init__("spanish.json", flags, settings, dateconfig)
...
class Portuguese(Locale):
    """---
    Represents the `Portuguese` context used for parsing lengths of time.
    ...
    def __init__(
        ...
        super().__init__("portuguese.json", flags, settings, dateconfig)

    def __repr__(self) -> str:
        """Return a string representation of the locale with attributes included."""
        return f"Portuguese(flags={self.flags!r}, settings={self.settings!r})"
...
class Guess:
    ...
    def __init__(
        ...
        self._flags: FailureFlags = flags
        self._settings: ParserSettings | None = settings
        self.locales: list[Locale] = [
            English(flags=flags, settings=settings, dateconfig=dateconfig),
            Spanish(flags=flags, settings=settings, dateconfig=dateconfig),
            Portuguese(flags=flags, settings=settings, dateconfig=dateconfig),
        ]

    @property
    def flags(self) -> FailureFlags:
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"Guess(flags={f'{self.flags!r}' if self.flags else None}, "
            f"settings={repr(self.settings) if self.settings else None})"
        )
