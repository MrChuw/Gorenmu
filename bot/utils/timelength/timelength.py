...
from bot.utils.timelength.locales import English, Guess, Locale
from bot.utils.timelength.parsers.date_parser import DateParserConfig

from .dataclasses import ParsedTimeLength, Scale
from .errors import (
    InvalidScaleError,
    NotALocaleError,
...
class TimeLength:
    ...
    - `parse()` — Parse `self.content` based on `self.locale`.
        ...
        - Manually call if changes are made to `self.content` or `self.locale`.
    ...
    def __init__(self, content: str, locale: Locale | Guess | None = None, dateconfig: DateParserConfig = None) -> None:
        ...
        self.locale: Locale = locale.locales[0] if isinstance(locale, Guess) else (locale or English(dateconfig))

        if not isinstance(self.locale, Locale):
            raise NotALocaleError(locale)

        self._result: ParsedTimeLength | None = None
    ...
    def parse(self, guess_locale: bool | Guess = False) -> None:
        ...
        if guess_locale is False:
            ...
            results.extend((self._parse_locale(locale), locale) for locale in guess.locales)
    ...
    def _resolve_base(self, base: datetime | None, use_date: bool) -> datetime:
        if use_date and self.result.date is not None:
            return self.result.date

        return base if base is not None else datetime.now(UTC)

    def ago(self, base: datetime | None = None, use_date: bool = False) -> datetime:
        """---
        ...
        resolved_base = self._resolve_base(base, use_date)

        if self._invalid_datetime(resolved_base, self.result.delta, subtract=True):
            raise PotentialDateTimeError

        return resolved_base - self.result.delta

    def hence(self, base: datetime | None = None, use_date: bool = False) -> datetime:
        """---
        ...
        if self._invalid_datetime(resolved_base, self.result.delta):
            raise PotentialDateTimeError

        return resolved_base + self.result.delta

    def to_date(self, base: datetime | None = None, use_date: bool = False) -> datetime:
        return self.hence(base=base, use_date=use_date)

    def to_timedelta(self):
        return self.result.delta

    def to_microseconds(self, max_precision: int | None = None) -> float:
        """---
        ...
        return self._round(self.locale.microsecond, max_precision)
    ...
    def to_seconds(self, max_precision: int | None = None) -> float:
        """---
        ...
        return self._round(self.locale.second, max_precision)
    ...
    @staticmethod
    def _invalid_datetime(date: datetime, delta: timedelta | datetime, subtract: bool = False) -> bool:
        ...
        min_datetime_timestamp = datetime(1, 1, 1, 0, 0, 0, 0, tzinfo=UTC).timestamp()
        max_datetime_timestamp = datetime(9999, 12, 31, 23, 59, 59, 999999, tzinfo=UTC).timestamp()
    ...
    @staticmethod
    def _invalid_timedelta(first: timedelta, second: timedelta, subtract: bool = False) -> bool:
    ...
    @staticmethod
    def _convert_to_hhmmss(seconds: int | float) -> str:
    ...
    def __add__(self, other: TimeLength | timedelta | float | int) -> TimeLength:
        ...
        if isinstance(other, TimeLength):
            ...
            return TimeLength(
                content=f"{abs(self.result.seconds + other) / base.scale} {base.terms[0]}", locale=deepcopy(self.locale)
            )
        else:
    ...
    def __rsub__(self, other: datetime | timedelta) -> datetime | timedelta:
        """
        Get the difference between a `datetime` or a `timedelta` and `self`.
    ...
    def __divmod__(self, other: TimeLength | timedelta | float | int) -> tuple[float, TimeLength]:
        ...
        if isinstance(other, TimeLength):
            return self.result.seconds // other.result.seconds, self % other
        elif isinstance(other, timedelta):
            return self.result.seconds // other.total_seconds(), self % other
        elif isinstance(other, (float, int)):
            return self.result.seconds // other, self % other
    ...
    def __rdivmod__(self, other: timedelta) -> tuple[float, timedelta]:
        ...
        if isinstance(other, timedelta):
            return other.total_seconds() // self.result.seconds, other % self
        raise NotImplementedError
    ...
    def __pow__(self, other: float | int, mod: TimeLength | timedelta | float | int | None = None) -> TimeLength:
        ...
        #### Returns
        - A `TimeLength` that represents the absolute power of `self`, and the passed number, optionally module by mod.

        #### Raises
        - `NoValidScalesError` when no valid and enabled scales are found to perform the action.
        """

        if not isinstance(other, (float, int)) or (
            mod is not None and not isinstance(mod, (TimeLength, timedelta, float, int))
        ):
            return NotImplemented
        ...
        return TimeLength(content=f"{result / base.scale} {base.terms[0]}", locale=deepcopy(self.locale))
    ...
    def __ge__(self, other: TimeLength | timedelta | float | int) -> bool:
        """
        Check if `self` is greater than or equal to others.
