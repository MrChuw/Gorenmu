...
class Alias(Model, TimestampMixin):
    ...
    indexes = [("user", "channel", "name"), ("channel",), ("command",), ("parent",)]  # NOQA: RUF012
    ...
    @staticmethod
    async def list_alias(ctx: Context, user_id: int | None = None, user: User = None) -> list[Alias]:
        return await ctx.bot.memcache.Alias.list(user_id=user_id or user.id)

    @staticmethod
    def filter_cached(ctx: Context, user: User = None, alias_name: str | None = None) -> CachedAliasQuerySet:
        return CachedAliasQuerySet(ctx=ctx, user=user, alias_name=alias_name)

    @staticmethod
    async def filter_first_prefetch(
        ctx: Context, user: User = None, alias_name: str | None = None, prefetch: str = "parent"
    ) -> Alias:
...
class CachedAliasQuerySet:  # TODO: The same thing with other caches...
    def __init__(self, ctx: Context, user: User, alias_name: str | None = None, deleted: bool = False):
        self.ctx = ctx
