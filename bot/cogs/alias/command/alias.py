...
from bot.ext import commands, Context, Command
from bot.models import Alias, User
from bot.translations import BaseDecorators, Response, BaseTranslations
...
# TODO: Replace all Alias.filter with Alias.filter_cached
...
class AliasCmd(commands.CustomComponent):
    ...
    async def add_command(self, ctx: Context, *args):
        ...
        if result.guard_result:
            return ctx.user.translations.Exceptions.guard_caught.format_response(
                    ...
                    result.guard_result,
                    ctx.bot.dev_name,
                    success=False, pipe=False
            )

        rest = [arg for arg in rest if arg]
        alias = await Alias.create_cached(ctx=ctx, name=name, command=result.command, invocation=command_string,
                                          arguments=rest)
    ...
    async def check_alias(self, ctx: Context, *args):  # remains in class
        ...
        if not target_aliases_flat and first_name not in aliases_flat and not second_name:
            return translations.user_has_no_alias.format_response(
                ctx, target_user.name, success=False, pipe=False
            )
        if not target_aliases_flat and first_name in aliases_flat and not second_name:
            ...
            return await handle_special_case(
                ctx, translations, aliases, target_aliases, first_name, target_user
            )
        ...
        if second_name:
            ...
            if not user:
                ...
                return exception.user_not_found_name.format_response(
                    ctx, first_name, success=False, pipe=False
                )
    ...
    async def link_alias(self, ctx: Context, *args):
        ...
        if not target_alias or (not target_alias.command and not target_alias.parent):
            ...
            return translations.Link.link_to_with_invalid_name.format_response(
                    ctx,
                    translations.alias_invalid_name,
                    success=False,
                    pipe=False
            )
...
async def upload_alias(ctx: Context, aliases: list[Alias], table_name: str):
    ...
    for alias in aliases:
        ...
        alias_table_list.append(
                {alias_table_headers[0]: name, alias_table_headers[1]: description, alias_table_headers[2]: invocation,
                 alias_table_headers[3]: arguments, alias_table_headers[4]: parent_name,
                 alias_table_headers[5]: updated_at, alias_table_headers[6]: created_at
                 }
        )
...
async def handle_list_all(ctx: Context, translations):
    ...
    url = await upload_alias(
        ctx, aliases, translations.alias_table_name.format(ctx.author.display_name)
    )
    return translations.Check.user_alias_list.format_response(
        ctx, ", ".join(names), url, success=False, pipe=False
    )
...
async def handle_target_list(ctx: Context, target_user, translations: TAlias):
    ...
    url = await upload_alias(
        ctx, target_aliases, translations.alias_table_name.format(mention)
    )
    return translations.Check.list_of_alias_of.format_response(
        ctx, mention, url, pipe=False
    )

async def handle_special_case(ctx: Context, translations: TAlias, aliases, target_aliases, first_name: str, target_user):
    url1 = await upload_alias(
        ctx, aliases, translations.alias_table_name.format(ctx.author.display_name)
    )
    url2 = await upload_alias(
        ctx, target_aliases, translations.alias_table_name.format(target_user.name)
    )
    return translations.Check.list_of_special_case.format_response(
        ctx, first_name, url1, url2, pipe=False
    )
...
async def handle_alias_lookup(ctx: Context, user, alias_name: str, translations: TAlias):
    ...
    if not alias:
        ...
        return translations.Check.alias_not_found.format_response(
            ctx, mention, alias_name, success=False, pipe=False
        )

    if not alias.command and not alias.parent:
        return translations.Check.alias_deleted.format_response(
            ctx, alias_name, success=False, pipe=False
        )

    url = await upload_alias(
        ctx, [alias], translations.alias_table_name.format(user.name)
    )
    ...
    return translations.Check.normal_message.format_response(
        ctx, alias.name, invocation, url, pipe=False
    )
