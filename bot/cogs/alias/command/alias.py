...
class AliasCmd(commands.CustomComponent):
    ...
    async def add_command(self, ctx: Context, *args):
        ...
        if not command_string:
            return translations.Add.no_command_to_add.format_response(ctx, ctx.prefix, success=False)
        ...
        if not result.command:
            return translations.Add.command_dont_exist.format_response(ctx, command_string, success=False)

        if result.guard_result:
            return ctx.user.translations.Exceptions.guard_caught.format_response(
                ctx, result.trigger_name, result.guard_result, ctx.bot.dev_name, success=False
            )
        ...
        return translations.Add.alias_created.format_response(ctx, alias.name)
    ...
    async def check_alias(self, ctx: Context, *args):
        ...
        if not target_aliases_flat and first_name not in aliases_flat and not second_name:
            return translations.user_has_no_alias.format_response(ctx, target_user.name, success=False)
        ...
        if second_name:
            ...
            if not user:
                ...
                return exception.user_not_found_name.format_response(ctx, first_name, success=False)
    ...
    async def copy_alias(self, ctx: Context, *args):
        ...
        if not target_user_name:
            return exception.user_not_provided.format_response(ctx, success=False)
        if not target_alias_name:
            return translations.Copy.alias_not_provided.format_response(ctx, success=False)
        if not ALIAS_NAME_REGEX.match(target_alias_name):
            return translations.Copy.target_alias_invalid_name.format_response(ctx, success=False)
        alias = await Alias.filter_cached(ctx=ctx, alias_name=target_alias_name)
        if alias:
            return translations.Add.alias_name_conflict.format_response(ctx, target_alias_name, success=False)

        target_user = await User.get_user_or_none(ctx, name=target_user_name)
        if not target_user:
            return exception.user_not_found_name.format_response(ctx, target_user_name, success=False)
        ...
        if target_alias is None:
            return translations.Copy.no_alias_found.format_response(
                ctx, target_alias_name, target_user.name, success=False
            )
        ...
        if target_alias.command is None and target_alias.parent:
            ...
            return translations.Copy.link_to_a_link.format_response(
                ctx, ctx.prefix, original_user.name, target_alias.parent.name, success=False
            )
        else:
        ...
        if new_name:
            return translations.Copy.copy_success.format_response(ctx, target_alias_name, new_alias.name)
    ...
    async def describe_alias(self, ctx: Context, *args):
        translations = ctx.user.translations.Alias
        if not args:
            return translations.Describe.no_args_to_parse.format_response(ctx, ctx.prefix, success=False)
        ...
        if not description or description.lower() == "none":
            ...
            await alias.save()
            return translations.Describe.description_reverted.format_response(ctx, name)
        else:
    ...
    async def edit_alias(self, ctx: Context, *args):
        translations = ctx.user.translations.Alias
        if len(args) < 2:
            return translations.Edit.no_args_provided.format_response(ctx, success=False)
        ...
        if command_check.guard_result:
            return ctx.user.translations.Exceptions.guard_caught.format_response(
                ctx, command_check.trigger_name, command_check.guard_result, ctx.bot.dev_name, success=False
            )
        ...
        if alias.command is None:  # NOQA
            return translations.Edit.edit_link.format_response(ctx, args, success=False)
        ...
        await alias.save()
        return translations.Edit.edit_success.format_response(ctx, alias.name)
    ...
    async def link_alias(self, ctx: Context, *args):
        ...
        original_user = None
        if len(args) < 2:
            return translations.Link.link_no_args.format_response(ctx, ctx.prefix, success=False)
        ...
        if existing_alias and not custom_link_name:
            return translations.Add.alias_name_conflict.format_response(ctx, alias_name, success=False)

        if existing_alias and existing_alias.name == custom_link_name:
            return translations.Link.alias_name_already_exists.format_response(ctx, alias_name, success=False)

        target_user_data = await User.get_or_none(name=user_name)
        if not target_user_data:
            return exception.user_not_found_name.format_response(ctx, user_name, success=False)
        ...
        if not target_alias or (not target_alias.command and not target_alias.parent):
            return translations.Link.user_dont_has_alias.format_response(ctx, alias_name, success=False)
            ...
            return translations.Link.link_to_with_invalid_name.format_response(
                ctx, translations.alias_invalid_name, success=False
            )
        elif custom_link_name and not ALIAS_NAME_REGEX.match(custom_link_name):
            return translations.Link.link_to_with_invalid_name.format_response(ctx, custom_link_name, success=False)
        ...
        if link_to_link:
            return translations.Link.link_to_link.format_response(
                ctx, target_alias.name, original_user.name, name_string
            )

        return translations.Link.link_success.format_response(ctx, name_string)
    ...
    async def remove_alias(self, ctx: Context, *args):
        translations = ctx.user.translations.Alias
        if not args:
            return translations.Remove.no_alias_name_provided.format_response(ctx, success=False)
        name, *rest = chain(args, repeat(None, 2))
    ...
    async def rename_alias(self, ctx: Context, *args):
        translations = ctx.user.translations.Alias
        if len(args) < 2:
            return translations.Rename.no_name_provided.format_response(ctx, success=False)
        ...
        if not old_alias:
            return translations.dont_have_alias.format_response(ctx, old_alias_name, success=False)
        existing_alias = await Alias.filter_cached(ctx=ctx, alias_name=new_alias_name).first()
        if existing_alias:
            return translations.Rename.alias_already_exists.format_response(ctx, new_alias_name, success=False)
...
async def handle_list_all(ctx: Context, translations):
    ...
    return translations.Check.user_alias_list.format_response(ctx, ", ".join(names), url, success=False)
...
async def handle_special_case(
    ...
    return translations.Check.list_of_special_case.format_response(ctx, first_name, url1, url2)
...
async def handle_alias_lookup(ctx: Context, user, alias_name: str, translations: TAlias):
    ...
    if not alias.command and not alias.parent:
        return translations.Check.alias_deleted.format_response(ctx, alias_name, success=False)
    ...
    if not alias.command and alias.parent:
        ...
        return translations.Check.appendix_message.format_response(
            ctx, parent_alias.name, original_user.name, invocation, url
        )

    return translations.Check.normal_message.format_response(ctx, alias.name, invocation, url)
