...
async def check_alias(ctx: Context, args: tuple):
    ...
    if not first_name and not second_name:
        ...
        return translations.Check.user_alias_list.format_response(ctx, username, success=False)
    ...
    if target_aliases == [] and first_name not in aliases:
        return translations.Check.no_alias_found.format_response(ctx, second_name, target_user.name, success=False)
        ...
        return translations.Check.list_of_alias_of.format_response(ctx, mention, url, success=True)
    else:
    ...
    if not alias.command and not alias.parent:  # NOQA
        return translations.Check.alias_deleted.format_response(ctx, mention, success=False)
    ...
    return translations.Check.final_message.format_response(ctx, message, url)
...
async def link_alias(ctx: Context, args: tuple):
    translations = ctx.translations.Alias
    if len(args) < 2:
        return translations.Link.link_no_args.format_response(ctx, success=False)
    ...
    if existing_alias:
        return translations.Link.alias_name_already_exists.format_response(ctx, success=False)
    ...
    return translations.Link.link_success.format_response(ctx, name_string, appendix)
