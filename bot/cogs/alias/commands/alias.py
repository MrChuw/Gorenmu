...
class AliasCmd(commands.CustomComponent):
    ...
    async def check_alias(self, ctx: Context, args: tuple):  # TODO: test check a link alias
        translations = ctx.user.translations.Alias
        ...
        if not target_aliases_flat and first_name not in aliases_flat and not second_name:
            ...
            mention = mention = ctx.user.translations.SupportTools.mention if user.name == ctx.author.name else f"@{user.name}"
        elif target_aliases_flat and first_name not in aliases_flat and not second_name:
            mention = ctx.user.translations.SupportTools.mention if target_user.name == ctx.author.name \
    ...
    async def describe_alias(self, ctx: Context, args: tuple):  # NOQA
        translations = ctx.user.translations.Alias
        if not args:
    ...
    async def edit_alias(self, ctx: Context, args: tuple):
        translations = ctx.user.translations.Alias
        if len(args) < 2:
    ...
    async def link_alias(self, ctx: Context, args: tuple):
        ...
        if not target_alias and not target_alias.parent:
            ...
            return translations.Link.link_with_invalid_name.format_response(
                    ctx,
    ...
    async def remove_alias(self, ctx: Context, args: tuple):
        translations = ctx.user.translations.Alias
        if not args:
    ...
    async def rename_alias(self, ctx: Context, args: tuple):
        translations = ctx.user.translations.Alias
        if len(args) < 2:
...
def parse_command_by_name(ctx: Context, string: str) -> commands.Command:
    prefix = ctx.prefix
...
async def upload_alias(ctx: Context, aliases: list[Alias], table_name: str):  # TODO: Fazer o upload
    ...
    response = await ctx.bot.UploadThings.shortener(
            response.url.human_repr(),
            ["aliases"],
            ctx.bot,
            alias_cached_session.session
    )
    return response
