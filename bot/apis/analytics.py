...
class Analytics:
    ...
    def _payload(
        self, *, text: str, id: str, name: str, locale: str, extra_json: dict = {}
    ) -> dict:
        user_json = {"firstName": name, "locale": locale}
        return {
            "userId": id,
            "text": text,
            "platformJson": extra_json,
            "platformUserJson": user_json,
        }

    async def received(
        self, author_id: int, author_name: str, channel_name: int, message: str
    ) -> None:
        ...
        await aiorequests.post(
            self.url, params=params, json=payload, raise_for_status=False, wait_response=False
        )
