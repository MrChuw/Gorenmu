...
def test_simple_translation(mock_requests):
    translator = BaiduTranslator(
        appid="this-is-an-valid-appid", appkey="this-is-an-valid-appkey", source="en", target="zh"
    )
...
def test_wrong_api_key(mock_requests):
    translator = BaiduTranslator(
        appid="this-is-a-wrong-appid", appkey="this-is-a-wrong-appkey", source="en", target="zh"
    )
    # Set the response status_code only.
    mock_response = Mock()
    mock_response.status_code = 200
    mock_response.json.return_value = {"error_code": "54001", "error_msg": "Invalid Sign"}
    mock_requests.post.return_value = mock_response
...
@pytest.mark.skipif(appid is None or appkey is None, reason="appid or appkey is not provided")
def test_baidu_successful_post_onetarget():
    posted = BaiduTranslator(appid=appid, appkey=appkey, source="en", target="zh").translate(
        "Hello! How are you?"
    )
