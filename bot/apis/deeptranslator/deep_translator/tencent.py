...
class TencentTranslator(BaseTranslator):
    ...
    def __init__(
        ...
        secret_key: Optional[str] = os.getenv(TENCENT_SECRET_KEY_ENV_VAR, None),
        **kwargs,
    ):
        """
        ...
        super().__init__(
            base_url=url, source=source, target=target, languages=TENCENT_LANGUAGE_TO_CODE, **kwargs
        )
    ...
    def translate(self, text: str, **kwargs) -> str:
        ...
        if is_input_valid(text):
            ...
            hmac_str = hmac.new(
                self.secret_key.encode("utf8"), (s + query_str).encode("utf8"), hashlib.sha1
            ).digest()
