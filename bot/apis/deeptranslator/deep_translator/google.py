...
class GoogleTranslator(BaseTranslator):
    ...
    def __init__(
        self, source: str = "auto", target: str = "en", proxies: Optional[dict] = None, **kwargs
    ):
        """
    ...
    async def translate(self, text: str, **kwargs) -> str:
        ...
        if is_input_valid(text, max_chars=5000):
            ...
            response = await self.sessao.get(
                self._base_url, params=self._url_params, proxy=self.proxies
            )
    ...
    async def translate_file(self, path: str, **kwargs) -> str:
        """
