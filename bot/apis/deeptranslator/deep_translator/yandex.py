...
from bot.apis.deeptranslator.deep_translator.constants import BASE_URLS, YANDEX_ENV_VAR
...
class YandexTranslator(BaseTranslator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.api_endpoints = {"langs": "getLangs", "detect": "detect", "translate": "translate"}
        super().__init__(base_url=BASE_URLS.get("YANDEX"), source=source, target=target, **kwargs)
    ...
    def detect(self, text: str, proxies: Optional[dict] = None):
        response = None
        params = {"text": text, "format": "plain", "key": self.api_key}
        try:
    ...
    def translate(self, text: str, proxies: Optional[dict] = None, **kwargs) -> str:
        if is_input_valid(text):
            ...
            try:
                url = self._base_url.format(version=self.api_version, endpoint="translate")
                response = requests.post(url, data=params, proxies=proxies)
            except ConnectionError:
                raise ServerException(503)
            else:
