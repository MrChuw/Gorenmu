...
class CLI(object):
    ...
    def parse_args(self) -> argparse.Namespace:
        ...
        parser = argparse.ArgumentParser(
            add_help=True, description="Official CLI for deep-translator", usage="dt --help"
        )
        ...
        parser.add_argument(
            "--source", "-src", default="auto", type=str, help="source language to translate from"
        )
        ...
        parsed_args = (
            parser.parse_args(self.custom_args) if self.custom_args else parser.parse_args()
        )
        return parsed_args
