...
from bot.apis.deeptranslator.deep_translator.validate import request_failed
...
class QcriTranslator(BaseTranslator):
    ...
    def _get(self, endpoint: str, params: Optional[dict] = None, return_text: bool = True):
        if not params:
            params = self.params
        try:
            res = requests.get(
                self._base_url.format(endpoint=self.api_endpoints[endpoint]), params=params
            )
