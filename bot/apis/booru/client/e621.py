...
class E621(object):
    ...
    def append_obj(raw_object: dict):
        ...
        for i in range(len(raw_object)):
            if "id" in raw_object[i]:
                raw_object[i]["post_url"] = f"{get_hostname(Booru.e621)}/posts/{raw_object[i]['id']}"
    ...
    async def search(
        ...
        while self.final == {"teste": 1}:
            ...
            async with CachedSession(cache=self.cache, timeout=timeout) as session:
                async with session.get(
                    Booru.e621, params=self.specs, headers=Booru.headers, allow_redirects=True
                ) as resp:
        ...
        self.not_random = E621.append_obj(self.final)
        ...
        try:
            if gacha:
                ...
                return better_object(E621.append_obj(self.final))
