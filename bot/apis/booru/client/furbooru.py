...
class Furbooru(object):
    ...
    def append_obj(raw_object: dict):
        ...
        for i in range(len(raw_object)):
            if "id" in raw_object[i]:
                raw_object[i]["post_url"] = f"{get_hostname(Booru.furbooru)}/images/{raw_object[i]['id']}"
    ...
    async def search(
        ...
        path = f"./data/"
        if self.specs["q"] == "":
            with open(path + "furbooru_tags.pickle", "rb") as fp:
                tags = pickle.load(fp)
        ...
        while self.final == {"teste": 1}:
            ...
            async with CachedSession(cache=self.cache, timeout=timeout) as session:
                async with session.get(Booru.furbooru, params=self.specs, allow_redirects=True) as resp:
        ...
        self.not_random = Furbooru.append_obj(self.final)
        ...
        try:
            if gacha:
                ...
                return better_object(Furbooru.append_obj(self.final))
