    ...
    def append_obj(raw_object: dict):
        ...
        for i in range(len(raw_object)):
            if "id" in raw_object[i]:
                raw_object[i]["post_url"] = f"{get_hostname(Booru.konachan_net)}/post/show/{raw_object[i]['id']}"
    ...
    async def search(
        ...
        while self.final == {"teste": 1}:
            ...
            async with CachedSession(cache=self.cache, timeout=timeout) as session:
                async with session.get(Booru.konachan_net, params=self.specs, allow_redirects=True) as resp:
