...
async def base_wikipedia(interact, mock_context: MockContext, expected):
...
async def test_two_hundred(interact, mock_context: MockContext, lang: str, expected: str):
    session = mock_context.bot.SessionsCaches.WikipediaCachedSession
    await mock_context.prepare_context(lang)
    async with mock_context.MockBuilder.Session.get_not_cached(session, "www.some_url.com"):
        await base_wikipedia(interact, mock_context, expected=expected)
