...
from bot.cogs.wikipedia.command.wikipedia import Response, WikipediaCmd
...
async def test_two_hundred(interact, mock_context: MockContext, lang: str, expected: str):
    ...
    with patch.object(
        mock_context.bot.SessionsCaches.WikipediaCachedSession, "get_not_cached", new_callable=AsyncMock
    ) as mock_get_scp:
...
async def test_timeout(interact, mock_context: MockContext, lang: str, expected: str):
    ...
    with (
        patch.object(
            mock_context.bot.SessionsCaches.WikipediaCachedSession,
            "get_not_cached",
