...
async def test_decorators(interact, mock_context: MockContext, lang: str, helper: str, usage: str):
    ...
    decorator = mock_context.bot.TranslationManager.get_decorator(interact.hypertranslate, mock_context)
    mock_context.Asserter.assert_string(decorator.usage, usage)
    mock_context.Asserter.assert_string(decorator.helper, helper)


async def base_nada(
    interact, mock_context: MockContext, lang: str, content: str, expected: str, quantity: int, pass_q: bool
):
    await mock_context.prepare_context(lang)
    with (
...
async def test_text(interact, mock_context: MockContext, lang: str, expected: str):
    await base_nada(interact, mock_context, lang=lang, content="blabla", expected=expected, quantity=10, pass_q=False)
...
async def test_text_and_quantity(interact, mock_context: MockContext, lang: str, expected: str):
    await base_nada(
        interact, mock_context, lang=lang, content="blablabla", expected=expected, quantity=100, pass_q=True
    )
