...
from bot.cogs.echo.command.echo import EchoCmd
...
async def test_decorators(interact, mock_context: MockContext, lang: str, helper: str, usage: str):
    ...
    mock_context.Asserter.assert_string(interact.translations.Echo.deco_usage(mock_context, "+"), usage, strict=True)
    mock_context.Asserter.assert_string(interact.translations.Echo.deco_helper(mock_context, "+"), helper, strict=True)
...
async def base_echo(
    ...
    await mock_context.prepare_context(lang)
    response: Response = await interact.echo._callback(interact, mock_context, content=content)  # NOQA
