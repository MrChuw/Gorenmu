...
from tests.tests.cogs.afk.templates import templates_afk_return as templates
...
async def test_afk_return_afk_no_content(interact, mock_context: MockContext):
    await templates.test_afk_return_afk_no_content(
        interact, mock_context, lang=lang, expected="você voltou 🏃⌨ (estava ausente por "
    )
...
async def test_afk_return_afk_content(interact, mock_context: MockContext):
    await templates.test_afk_return_afk_content(
        interact, mock_context, lang=lang, expected="você voltou 🏃⌨ e deixou uma nota: content (estava ausente por"
    )
