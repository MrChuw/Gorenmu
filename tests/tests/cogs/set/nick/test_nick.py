...
async def test_decorators(interact, mock_context: MockContext, lang: str, helper: str, usage: str):
    ...
    decorator = mock_context.bot.TranslationManager.get_decorator(interact.set_nick, mock_context)
    mock_context.Asserter.assert_string(decorator.usage, usage)
    mock_context.Asserter.assert_string(decorator.helper, helper)


async def base_nick(interact, mock_context: MockContext, lang: str, args: str, expected: str):
    await mock_context.prepare_context(lang)
...
async def test_nick_set(interact, mock_context: MockContext, lang: str, args: str, expected: str):
    await base_nick(interact, mock_context, lang, args, expected)
