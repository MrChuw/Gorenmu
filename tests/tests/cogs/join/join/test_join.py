...
from bot.cogs.join.command.join import JoinCmd
...
async def base_join(
    ...
    await mock_context.prepare_context(lang, interact=interact)
    response: Response = await interact.join._callback(interact, mock_context)  # NOQA
...
async def test_no_content(interact, mock_context: MockContext, lang: str, expected: str):
    await base_join(interact, mock_context, lang=lang, expected=expected, success=True)
