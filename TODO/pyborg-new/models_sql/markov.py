...
class Word(Model):
    ...
    async def greb_lines_and_indexs(self):
        results = []
        lines = await self.lines.all()
        indexs = self.indexs

        for i in range(len(indexs)):
            index = indexs[i]
            line = [line for line in lines if line.hashval == index["hashval"]][0]
            results.append((line, index["index"]))
        return results

    @staticmethod
    async def get_word_list(word_list: list[str]) -> 'Word':
        for word in word_list:
            word = await Word.filter(word=word).first()
            if not word:
                continue
            return word

    @staticmethod
    async def random_word() -> str:
        words = await Word.all().count()
        choice = random.randint(0, words - 1)
        return (await Word.get(id=choice)).word
