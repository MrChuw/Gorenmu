...
from typing import Any, Callable, cast, Dict, List, MutableMapping, Optional, overload, Union

import cogs.profile.commands.mention
        ...
        if random_reply or self.user.mentioned_in(message) or self._plaintext_name(message):
            async with message.channel.typing():
                ...
                if msg:
                    ...
                    msg = msg.replace("#nick", str(cogs.profile.commands.mention.mention))
        ...
        if self.settings['pyborg']['multiplex']:
            ...
            async with self.aio_session.post(url, data={"body": body}, raise_for_status=True) as ret:
                try:
                    ...
                    await self.change_presence(status=models.user.Activity.Status.idle, activity=discord.Game("Error, see logs"))
            return reply
        else:
            raise NotImplementedError
